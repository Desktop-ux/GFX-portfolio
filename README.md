...
- **Adding a short-form video**: in `index.html`, find `<!-- NEW-SHORT-CARD-MARKER -->` inside `#shortsTrack`, duplicate the `.short-card` block above it, drop your file in `assets/videos/`, then add `src="assets/videos/yourfile.mp4"` to that card's `<video>` tag. That's it — the gradient placeholder hides itself automatically once a real source is detected.
- **Adding a long-form video**: same idea — find `<!-- NEW-LF-ROW-MARKER -->` inside `#longformList`, duplicate a `.lf-row` block, and add the `src` to its `<video>` tag.
