...
/* ---------- video cards: auto-detect a real source and swap
   the gradient placeholder for the actual video ---------- */
function initVideoCards(){
  document.querySelectorAll('.short-visual, .lf-visual').forEach(wrap=>{
    const vid = wrap.querySelector('video');
    const ph  = wrap.querySelector('.ph');
    if(!vid) return;
    const hasSource =
      (vid.getAttribute('src') && vid.getAttribute('src').trim() !== '') ||
    ...
    if(hasSource){
      ...
      vid.style.display = 'none'; // nothing to show — keep the gradient placeholder
    }
  });
}
initVideoCards();
